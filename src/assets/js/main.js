...
function opening() {
  ...
  document.addEventListener('mouseup', (ev) => {if(ev.button == 1) alert('Creado por JuanM04')})
  anime.timeline()
    .add({
      targets: 'h1',
      translateY: [
        {value: r(-750), duration: 0},
...
function startGame(first) {
  let translateY = first ? r(-state.gameX) : [{value: r(state.gameX), easing: 'easeInExpo'}, {value: r(-state.gameX), delay: 1000}]
  anime({
    targets: '#game',
    translateY,
    duration: 2000,
...
function r(n) {
  return window.innerHeight * n / 1440
}
